...
```json
{
  ...
  "settings": {"games_per_pair": 1, "max_matches_per_team_per_week": 1, "min_rest_hours": 24}
}
```

`allowed_slot_ids` is a hard constraint; an empty list means the team may play in every submitted slot. `preferred_slot_ids` is optimized but never makes a schedule impossible.
