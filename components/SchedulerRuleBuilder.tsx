...
function RuleCard({ rule, onChange, onRemove }: { rule: SchedulerRule; onChange: (rule: SchedulerRule) => void; onRemove: () => void }) {
  ...
  return <article className="rounded-md border border-[#d6ded5] bg-white p-4"><div className="flex items-start justify-between gap-4"><div><h4 className="font-semibold text-[#18211c]">{title}</h4><p className="mt-1 text-sm text-[#637066]">{ruleSummary(rule)}</p></div><button type="button" onClick={onRemove} aria-label={`Remove ${title}`} className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-md border border-[#d7ded7] text-[#637066] hover:text-[#9a3d31]"><Trash2 className="h-4 w-4" /></button></div>
    {rule.type === "avoid_dates" ? <AvoidDatesFields rule={rule} onChange={onChange} /> : <label className="mt-4 block max-w-xs text-sm font-medium text-[#39433d]">{rule.type === "games_per_pair" ? "Games" : rule.type === "min_rest_hours" ? "Hours" : "Maximum games"}<input type="number" min={rule.type === "min_rest_hours" ? 0 : 1} max={rule.type === "games_per_pair" ? 10 : rule.type === "min_rest_hours" ? 168 : rule.type === "max_matches_per_team_per_week" ? 7 : 4} value={rule.value} onChange={(event) => onChange({ ...rule, value: Number(event.target.value) } as SchedulerRule)} className="mt-1 h-10 w-full rounded border border-[#cfd8d0] px-3" /></label>}
  </article>;
}

function AvoidDatesFields({ rule, onChange }: { rule: Extract<SchedulerRule, { type: "avoid_dates" }>; onChange: (rule: SchedulerRule) => void }) {
  function updateDate(index: number, value: string) {
    onChange({ ...rule, dates: rule.dates.length ? rule.dates.map((date, currentIndex) => currentIndex === index ? value : date) : [value] });
  }

  return <div className="mt-4 grid gap-3 sm:grid-cols-2"><div className="sm:col-span-2"><div className="flex items-center justify-between gap-3"><p className="text-sm font-medium text-[#39433d]">Dates</p><button type="button" onClick={() => onChange({ ...rule, dates: [...rule.dates, ""] })} className="text-sm font-semibold text-[#1f5b47]">Add date</button></div><div className="mt-2 space-y-2">{(rule.dates.length ? rule.dates : [""]).map((date, index) => <div key={index} className="flex gap-2"><input required type="date" value={date} onChange={(event) => updateDate(index, event.target.value)} className="h-10 min-w-0 flex-1 rounded border border-[#cfd8d0] px-3" />{rule.dates.length > 1 ? <button type="button" onClick={() => onChange({ ...rule, dates: rule.dates.filter((_, currentIndex) => currentIndex !== index) })} aria-label={`Remove date ${index + 1}`} className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded border border-[#d7ded7] text-[#637066] hover:text-[#9a3d31]"><Trash2 className="h-4 w-4" /></button> : null}</div>)}</div></div><label className="text-sm font-medium text-[#39433d]">Enforcement<select value={rule.strength} onChange={(event) => onChange({ ...rule, strength: event.target.value as "hard" | "soft" })} className="mt-1 h-10 w-full rounded border border-[#cfd8d0] bg-white px-3"><option value="hard">Never schedule</option><option value="soft">Avoid when possible</option></select></label></div>;
}
