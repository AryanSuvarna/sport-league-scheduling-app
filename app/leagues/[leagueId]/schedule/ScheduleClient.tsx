...
export function ScheduleClient({ initialData }: Props) {
  ...
  </div>{issuesOpen ? <IssuesPanel issues={data.issues} matches={data.matches} onClose={() => setIssuesOpen(false)} onSelect={focusIssue} /> : null}{selected ? <MatchDrawer match={selected} data={data} issues={issueByMatch.get(selected.id) ?? []} onClose={() => setSelectedId(null)} onSaved={() => { void refresh(); toast.success("Match changes saved."); }} onMessage={(value) => { if (value) toast.error(value); }} /> : null}{versionsOpen ? <ScheduleVersionsDialog leagueId={data.league.id} onClose={() => setVersionsOpen(false)} onRestored={refresh} /> : null}</main>;
}
